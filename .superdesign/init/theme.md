...
| Token | Value | Role |
|---|---|---|
| `--cream` | `#fff2ec` | Page/card cream |
| `--cream-soft` | `#f1e6dd` | Soft section fill, nav scrolled |
| `--cream-mid` | `#e8d9cd` | Borders, hairlines |
| `--brown-dark` | `#45301d` | Primary text, primary button |
| `--brown-mid` | `#5c4a32` | Secondary text, ghost borders |
| `--brown-light` | `#8b7258` | Muted labels, footer |
| `--olive` | `#74862f` | Brand accent, labels, badges |
| `--olive-dim` | `#5e6c23` | Darker olive |
| `--olive-line` | `#6b7238` | Hairline accent (lists, calendar dots) |
| `--olive-muted` | `#5c6230` | Muted olive |
| `--white` | `#ffffff` | Surfaces |
| `--shadow` | `rgba(69, 48, 29, 0.13)` | Card shadow |
| `--hero-ink` | `#1a120d` | Dark fallback / html background |

Hero overlay: dark brown/black gradients `rgba(12,8,5,0.38)` → `rgba(8,5,3,0.58)` over photography. Hero eyebrow/rotating word: `rgba(200,206,160,0.95)` / `rgba(220,225,185,0.98)`.

Admin extras: `--admin-page-bg: #f7f2ed`, `--admin-radius: 16px`, `--admin-nav-h: 64px`, `--admin-sidebar-w: 240px`.

### Typography
| Token | Family | Use |
|---|---|---|
| `--ff-display` / `--ff-brand` | `"Space Grotesk", sans-serif` | Wordmark, prices, quote italic |
| `--ff-airy` / `--ff-body` | `"Montserrat", sans-serif` | Body, headings (weight 200–400), labels |
Google Fonts also load Manrope (unused on public landing). Body: weight 300, letter-spacing 0.02em.

Type scale (approx):
- Hero title: `clamp(2.1rem, 5.5vw, 3.55rem)` weight 200, lowercase
- Section title: `clamp(1.7rem, 3.5vw, 2.5rem)` weight 300
- Section label: 0.8rem, weight 700, letter-spacing 0.15em, uppercase, olive
- Body: ~0.9–0.97rem, line-height 1.6–1.7
- Buttons: 1rem weight 600

### Spacing, radius, shadow, breakpoints
- Section padding: 90px 40px (60px 22px ≤700px)
- Nav padding: 18px 40px (16px 22px mobile)
- `--radius: 18px`, `--radius-sm: 10px`
- Buttons: pill `border-radius: 999px`, padding 14px 30px
- Shadows: brown-tinted, e.g. `0 8px 22px rgba(69,48,29,.22)` on primary CTA; glass cards `0 10px 36px rgba(40,28,18,0.1)`
- Breakpoints: **700px** (nav hamburger, spotlight 1-col, values stack), **640px** (pricing 1-col)

### Motion
- Reveal: opacity 0 + translateY(28px) → visible
- Hero title 1.35s cubic-bezier(0.2, 0.7, 0.25, 1)
- Rotating word 440ms; prefers-reduced-motion honored
- CTA shimmer on hero primary; accordion height 0.38s
- Hover: buttons translateY(-2px); cards -5px to -7px

### Layout structure
- Fixed transparent nav over full-bleed photo hero (100svh)
- Opaque cream sections: Practices (spotlight), Schedule, Testimonials, Pricing, Footer
- Glass sections: Values, Quotes (see-through to fixed `.site-bg` photo)
- Max content width: 1060px; calendar/filters ~520px

## Part 2 — Raw source dumps

### `assets/css/styles.css` — `:root` tokens
```css
/* ── TOKENS ── */
:root {
  --cream:      #fff2ec;
...
### `assets/css/styles.css` — full file
```css
/* ── TOKENS ── */
:root {
  --cream:      #fff2ec;
...
### `assets/css/testimonials.css` — full file
```css
/* ── TESTIMONIALS ── */
.testimonials {
  position: relative;
...
### `assets/css/admin.css` — extra tokens only
```css
/* Admin — inherits tokens from styles.css via admin page links */

/* ── EXTRA TOKENS (admin-only) ── */
:root {
  --admin-sidebar-w: 240px;
  --admin-nav-h: 64px;
  --admin-accent: var(--olive);
  --admin-accent-dim: rgba(116, 134, 47, 0.12);
  --admin-panel-bg: rgba(255, 255, 255, 0.78);
  --admin-panel-border: rgba(232, 217, 205, 0.7);
  --admin-input-bg: rgba(241, 230, 221, 0.55);
  --admin-input-focus: rgba(116, 134, 47, 0.22);
  --admin-shadow-xs: 0 1px 3px rgba(69, 48, 29, 0.07), 0 1px 2px rgba(69, 48, 29, 0.04);
  --admin-shadow-sm: 0 4px 16px rgba(69, 48, 29, 0.10), 0 2px 4px rgba(69, 48, 29, 0.05);
  --admin-shadow-md: 0 8px 28px rgba(69, 48, 29, 0.13), 0 2px 8px rgba(69, 48, 29, 0.07);
  --admin-shadow-focus: 0 0 0 3px rgba(116, 134, 47, 0.22);
  --admin-page-bg: #f7f2ed;
  --admin-radius: 16px;
